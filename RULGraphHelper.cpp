    ...
    const CString& title)
{
    ...
    for (int i = 0; i <= 8; i++)
    {
        float yPos = graphRect.Y + graphRect.Height * i / 8.0f;
        double value = 1.0 - (2.0 * i / 8.0);
    ...
    if (rul_mean >= x_min && rul_mean <= x_max)
    {
        ...
        Gdiplus::Font rulFont(L"¸¼Àº °íµñ", 9, FontStyleBold);
...
void RULGraphHelper::DrawLegend(CDC* pDC, CRect rect, int ci)
{
    ...
    Gdiplus::Font font(L"¸¼Àº °íµñ", 10, FontStyleRegular);
    SolidBrush textBrush(Color(255, 0, 0, 0));
    StringFormat format;
    ...
    double true_val, int ci, const CString& title)
{
    ...
    float col2X = legendX + 165.0f;
    float lineLen = 30.0f;
    float spacing = 15.0f;

    Gdiplus::Font legendFont(L"¸¼Àº °íµñ", 10);
